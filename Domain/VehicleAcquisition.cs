...
public sealed class VehicleAcquisitionSnapshot
{
    ...
    [DataMember(Name = "industrialTransportPolicies", Order = 36)] public List<IndustrialTransportPolicy> IndustrialTransportPolicies { get; set; } = new List<IndustrialTransportPolicy>();
    [DataMember(Name = "industrialTransportNeeds", Order = 37)] public List<IndustrialTransportNeed> IndustrialTransportNeeds { get; set; } = new List<IndustrialTransportNeed>();
    [DataMember(Name = "industrialCargoTags", Order = 38)] public List<IndustrialCargoTag> IndustrialCargoTags { get; set; } = new List<IndustrialCargoTag>();
}
...
public static class VehicleAcquisitionPersistence
{
    ...
    public static void Validate(VehicleAcquisitionSnapshot s)
    {
        ...
        foreach (var cost in s.OperatingCosts) if (cost == null || string.IsNullOrWhiteSpace(cost.SessionId) || string.IsNullOrWhiteSpace(cost.Fingerprint) || string.IsNullOrWhiteSpace(cost.AssetId) || cost.Payer == null || !Enum.IsDefined(typeof(OperatingCostSettlementMode), cost.SettlementMode) || cost.MaximumAuthorizedCost < 0 || cost.ReservedAmount < 0 || cost.ReservedAmount > cost.MaximumAuthorizedCost || cost.VanillaBalanceBefore < 0 || cost.VanillaBalanceAfter < 0 || cost.ActualCost < 0 || cost.ExternalReimbursement < 0 || cost.SubsidizedExcess < 0 || cost.SubsidizedExcess > cost.ActualCost || cost.ConditionBefore < 0m || cost.ConditionBefore > 1m || cost.ConditionAfter < 0m || cost.ConditionAfter > 1m || !s.Assets.Assets.Any(a => a.AssetId == cost.AssetId) || (cost.Payer.Kind == AccountKind.Player && cost.SettlementMode == OperatingCostSettlementMode.PersonalExternalWallet && cost.ReservedAmount != 0) || (cost.State == OperatingCostState.Open && (cost.Payer.Kind == AccountKind.Company || cost.SettlementMode == OperatingCostSettlementMode.SharedExternalWallet) && cost.MaximumAuthorizedCost > 0 && (cost.ReservedAmount != cost.MaximumAuthorizedCost || cost.ReservationReleased)) || (cost.State != OperatingCostState.Open && cost.ReservedAmount > 0 && !cost.ReservationReleased)) throw new InvalidDataException("Invalid operating cost record.");
    }
    ...
    private static void Migrate(VehicleAcquisitionSnapshot snapshot)
    {
        if (snapshot.Schema == VehicleAcquisitionSnapshot.CurrentSchema && snapshot.SchemaVersion >= 1 && snapshot.SchemaVersion < VehicleAcquisitionSnapshot.CurrentVersion)
        {
            ...
            CompanyEconomyPersistence.Migrate(snapshot.Economy);
            ...
            snapshot.IndustrialTransportPolicies = snapshot.IndustrialTransportPolicies ?? new List<IndustrialTransportPolicy>();
            snapshot.IndustrialTransportNeeds = snapshot.IndustrialTransportNeeds ?? new List<IndustrialTransportNeed>();
            snapshot.IndustrialCargoTags = snapshot.IndustrialCargoTags ?? new List<IndustrialCargoTag>();
